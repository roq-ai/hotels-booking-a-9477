...
import { useRoqClient, usePropertyFindFirst } from 'lib/roq';
...
function PropertyEditPage() {
  ...
  const { data, error, isLoading, mutate } = usePropertyFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: PropertyInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.property.update({
        data: values as RoqTypes.property,
        where: {
          id,
        },
      });
